...
import { CerrarLista, RemoverAlquilerBoton, BorrarTodosAlquileresBoton, AgregarAlquilerBoton } from './buttons';
import { useWatchlistContext } from '../context/WatchlistContext';
import { useTheme } from '../context/ThemeContext';

const WatchlistModal = ({ onClose }) => {
  const { verLista, alquilarPelicula, removerPelicula, removerPeliculas, calcularTotal } = useWatchlistContext();
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50">
      <div className={oscuro
        ...
        <h2 className={`text-2xl font-bold mb-4 ${oscuro ? 'text-white' : 'text-white'}`}>Carrito de Alquiler</h2>
              ...
              {verLista.map(pelicula => (
                ...
                >
                  <div className="relative">
                    <img src={pelicula.poster} alt={pelicula.title} className="w-20 h-28 object-cover rounded-2xl" />
                    <div className={oscuro
                  ...
                  <div className="flex-1">
                    <h3 className={`text-lg font-semibold ${oscuro ? 'text-white' : 'text-white'}`}>{pelicula.title}</h3>
                    <p className={oscuro ? 'text-yellow-300' : 'text-yellow-200'}>🛒 En carrito</p>
                    <p className={`text-sm ${oscuro ? 'text-gray-300' : 'text-yellow-100'}`}>${pelicula.price} ARS c/u</p>
                    <p className={`text-sm font-semibold ${oscuro ? 'text-yellow-300' : 'text-yellow-100'}`}>Subtotal: ${pelicula.price * pelicula.cantidad} ARS</p>
                  </div>
                  <AgregarAlquilerBoton onClick={() => alquilarPelicula(pelicula)} />
                  <RemoverAlquilerBoton onClick={() => removerPelicula(pelicula.id)} />
                </li>
              ))}
            </ul>
          ) : (
            <p className={oscuro ? "text-gray-400" : "text-gray-400"}>No tienes Alquileres.</p>
          )}
        </div>
        {verLista.length > 0 && (
          <div className={oscuro
            ...
            <div className="flex justify-between text-lg font-bold">
              <span className="text-white">Total ({totales.cantidad} películas):</span>
              <span className={oscuro ? "text-yellow-200" : "text-yellow-100"}>${totales.total} ARS</span>
            </div>
          </div>
        )}
